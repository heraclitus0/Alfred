...
MEMORY_FILE = os.path.join(
    os.path.dirname(__file__), "..", "..", "data", "claustrum_memory.json"
)
...
class ClaustumMemory:
    ...
    Every thought, observation, and conversation
    is written to disk immediately.
    
    Structure:
    {
        "identity": { name, created_at, admin, session_count },
        "observations": [ {time, text}, ... ],
        "thoughts": [ {time, tick, text}, ... ],
        "conversations": [ {time, role, text}, ... ],
        "patterns": [ {time, text, confidence}, ... ],
        "self_model": { ... what Claustrum knows about itself }
    }
    """
    ...
    def save_pattern(self, text: str, confidence: float = 0.5) -> None:
        """Save a detected pattern about the admin."""
    ...
    def update_self_model(self, key: str, value: Any) -> None:
        """Claustrum updates its model of itself."""
    ...
    def _write(self) -> None:
        try:
            ...
            print(f"[CLAUSTRUM memory write error: {e}]")
